...
TShortway::TShortway(TCreature *Creature, int VisibleX, int VisibleY){
	if(Creature == NULL){
		error("TShortway::TShortway: Passed creature is NULL.\n");
		return;
	}
	...
	for(int Y = -this->VisibleY; Y <= this->VisibleY; Y += 1){
		...
		if(Obj.exists()){
			...
			if(ObjType.getFlag(BANK) && !ObjType.getFlag(UNPASS)){
				...
				if(Waypoints == 0){
					error("TShortway::FillMap: Invalid waypoint value %d for bank %d.\n",
...
void TShortway::Expand(TShortwayPoint *Node){
	if(Node == NULL){
		error("TShortway::Expand: Passed node is NULL.\n");
		return;
	}
		...
		if(NeighborWaylength < Neighbor->Waylength){
			...
			if((Neighbor->x != 0 || Neighbor->y != 0) && Neighbor->Waypoints != -1){
				...
				if(Neighbor->Heuristic != INT_MAX){
					...
					if(Cur != Neighbor){
						error("TShortway::Expand: Node is not in ExpandList.\n");
					}else if(Prev == NULL){
			...
			switch(TD.Code){
				...
				case TDTalk:{
					...
					if(Text != NULL){
						...
						error("TCreature::Execute: Text is NULL at %s.\n", this->Name);
					}
					break;
				}
...
void TCreature::ToDoTalk(int Mode, const char *Addressee, const char *Text, bool CheckSpamming){
	...
	if(Text == NULL || Text[0] == 0){
		error("TCreature::ToDoTalk: Text is NULL at %s.\n", this->Name);
...
void TCreature::NotifyGo(void){
	...
	if(Bank != NONE){
		...
		error("TCreature::NotifyGo: There is no bank on field [%d,%d,%d].\n",
				DestX, DestY, DestZ);
	}
}
