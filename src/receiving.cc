...
bool CommandAllowed(TConnection *Connection, int Command){
	...
	if(Connection->State == CONNECTION_GAME){
		...
		error("CommandAllowed: Invalid connection status %d at command %d.\n",
...
void CTalk(TConnection *Connection, TReadBuffer *Buffer){
	...
	TPlayer *Player = Connection->GetPlayer();
	if(Player == NULL){
		print(3, "CTalk: No player is assigned to the connection.\n");
		return;
	}
	...
	if(Mode != TALK_SAY
		...
		print(3, "CTalk: Unknown speech mode %d.\n", Mode);
		return;
	}
	...
	if(Mode == TALK_PRIVATE_MESSAGE
		...
		if(Addressee[0] == 0){
			print(3, "CTalk: Addressee not specified.\n");
			return;
		}
	}
	...
	Buffer->readString(Text, sizeof(Text));
	if(Text[0] == 0){
		print(3, "CTalk: No text.\n");
		return;
	}

	if(findFirst(Text, '\n') != NULL){
		error("CTalk: %s uses newlines (%d,%s)\n", Player->Name, Mode, Text);
		return;
	}
	...
	if(Mode == TALK_CHANNEL_CALL
		...
		if(Channel == CHANNEL_RULEVIOLATIONS){
			print(3, "ChannelCall for GM request queue.\n");
			return;
		}
...
void CJoinChannel(TConnection *Connection, TReadBuffer *Buffer){
	...
	if(Channel < GetNumberOfChannels()){
		try{
			...
			print(3, "PLayer may not hear channel.\n");
		}
	}
}
...
void CSetTactics(TConnection *Connection, TReadBuffer *Buffer){
	...
	if(SecureMode != SECURE_MODE_DISABLED
		...
		print(3, "CSetTactics: Invalid security mode %d.\n", SecureMode);
		return;
	}
...
void CRevokeInvitation(TConnection *Connection, TReadBuffer *Buffer){
	...
	if(Player->GetPartyLeader(false) == 0){
		print(3, "Player is not a member of a hunting group.\n");
		return;
	}
...
void ReceiveData(TConnection *Connection){
	...
	if((Connection->InDataSize + 2) > (int)sizeof(Connection->InData)){
		error("ReceiveData: Too large packet length %d.\n", Connection->InDataSize);
		return;
	}
	...
	try{
		...
		error("ReceiveData: Error while reading command (%s).\n", str);
		return;
	}
	...
	try{
		...
		error("Error while reading data for command %d (%s).\n", Command, str);
	}
}
