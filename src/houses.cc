...
int CheckAccessRight(const char *Rule, TPlayer *Player){
	...
	if(Player == NULL){
		error("CheckAccessRight: Player is NULL.\n");
		return 0; // NOT_APPLICABLE ?
	}
...
bool IsSubowner(uint16 HouseID, TPlayer *Player, int TimeStamp){
	...
	if(Player == NULL){
		error("IsSubowner: Player is NULL.\n");
		return false;
	}
...
bool IsInvited(uint16 HouseID, TPlayer *Player, int TimeStamp){
	if(Player == NULL){
		error("IsInvited: Player is NULL.\n");
		return false;
	}
...
void ShowSubownerList(uint16 HouseID, TPlayer *Player, char *Buffer){
	...
	if(Player == NULL){
		error("ShowSubownerList: Player is NULL.\n");
		throw ERROR;
	}
	...
	print(3, "Editing subowner list of house %d.\n", HouseID);
...
void ShowGuestList(uint16 HouseID, TPlayer *Player, char *Buffer){
	...
	if(Player == NULL){
		error("ShowGuestList: Player is NULL.\n");
		throw ERROR;
	}
	...
	print(3, "Editing guestlist of house %d.\n", HouseID);
...
void ChangeSubowners(uint16 HouseID, TPlayer *Player, const char *Buffer){
	...
	if(Player == NULL){
		error("ChangeSubowners: Player is NULL.\n");
		throw ERROR;
	}
	...
	Log("houses", "%s changes list of subowners of house %d.\n", Player->Name, HouseID);
...
void ChangeGuests(uint16 HouseID, TPlayer *Player, const char *Buffer){
	...
	if(Player == NULL){
		error("ChangeGuests: Player is NULL.\n");
		throw ERROR;
	}
	...
	Log("houses", "%s changes guestlist of house %d.\n", Player->Name, HouseID);
...
void KickGuest(uint16 HouseID, TPlayer *Guest){
	...
	if(Bed != NONE && Bed.getAttribute(TEXTSTRING) != 0){
		print(3, "Player is standing on a bed while being kicked out of the house.\n");
		try{
			UseObjects(0, Bed, Bed);
		}catch(RESULT r){
			error("KickGuest: Exception %d while cleaning the bed.\n", r);
		}
	}
...
bool MayOpenDoor(Object Door, TPlayer *Player){
	...
	if(Player == NULL){
		error("MayOpenDoor: Player is NULL.\n");
		return false;
	}
...
void ShowNameDoor(Object Door, TPlayer *Player, char *Buffer){
	...
	if(Player == NULL){
		error("ShowNameDoor: Player is NULL.\n");
		throw ERROR;
	}
	...
	if(HouseID == 0){
		error("ShowNameDoor: Door at cordinate [%d,%d,%d] does not belong to any house.\n",
				DoorX, DoorY, DoorZ);
		throw ERROR;
	}

	if(!IsOwner(HouseID, Player)){
		print(3, "Player %s is not the owner of house %d.\n",
				Player->Name, HouseID);
		throw NOTACCESSIBLE;
	}
...
void ChangeNameDoor(Object Door, TPlayer *Player, const char *Buffer){
	...
	if(Player == NULL){
		error("ChangeNameDoor: Player is NULL.\n");
		throw ERROR;
	}
	...
	Log("houses", "%s changed NameDoor of house %d at coordinate [%d,%d,%d].\n",
...
void CleanHouse(THouse *House, TPlayerData *PlayerData){
	...
	if(PlayerData == NULL){
		...
		if(PlayerData == NULL){
			error("CleanHouse: Cannot assign slot for player data.\n");
			return;
		}
...
bool FinishAuctions(void){
	Log("houses","Processing ended auctions...\n");
	...
	for(int AuctionNr = 0; AuctionNr < NumberOfAuctions; AuctionNr += 1){
		...
		Log("houses", "Auction for house %d ended: Won by %s for %d gold.\n",
				HouseID, CharacterName, Bid);
		Log("houses", "At reset: INSERT INTO HouseAssignments VALUES ('%s',%d,%d,%d);\n",
		...
		if(PlayerData == NULL){
			error("FinishAuctions: Cannot assign slot for player data.\n");
			continue;
		}
		...
		if(DepotMoney < (House->Rent + Bid)){
			Log("houses", "Bidder does not have enough money.\n");
			House->OwnerID = CharacterID;
			strcpy(House->OwnerName, CharacterName);
			House->PaidUntil = 0;
		}else{
			Log("houses", "Bidder has enough money.\n");
	...
	for(int HouseNr = 0; HouseNr < Houses; HouseNr += 1){
		...
		if(H->PaidUntil == 0 && H->OwnerID != 0){
			if(QueryManagerConnection->excludeFromAuctions(H->OwnerID, true) != 0){
				error("FinishAuctions: Cannot ban bidder.\n");
			}
...
bool TransferHouses(void){
	...
	for(int TransferNr = 0; TransferNr < NumberOfTransfers; TransferNr += 1){
		...
		if(NewOwnerID != 0 && TransferPrice > 0){
			...
			if(PlayerData == NULL){
				error("CollectRents: Cannot assign slot for player data (1a).\n");
				continue;
			}
			...
			if(DepotMoney < TransferPrice){
				...
				Log("houses", "At reset: UPDATE HouseOwners SET Termination=null,NewOwner=null,Accepted=0,Price=null WHERE World=\'%s\' AND HouseID=%d;\n",
						HelpWorld, HouseID);
			...
			PlayerData = AssignPlayerPoolSlot(House->OwnerID, false);
			if(PlayerData == NULL){
				error("CollectRents: Cannot assign slot for player data (1b).\n");
				continue;
			}
		...
		// to a new owner?
		Log("houses", "Clearing house %d from %u.\n", HouseID, House->OwnerID);
		if(NewOwnerID == 0){
		...
		if(NewOwnerID != 0){
			...
			House->LastTransition = TimeStamp;
			Log("houses", "At reset: UPDATE HouseOwners SET Termination=%d,NewOwner=%u,Accepted=1,Price=%d WHERE World='%s' AND HouseID=%d;\n",
					TimeStamp, NewOwnerID, TransferPrice, HelpWorld, HouseID);
		}else{
			Log("houses", "At reset: UPDATE HouseOwners SET Termination=%d,NewOwner=null,Accepted=0,Price=0 WHERE World=\'%s\' AND HouseID=%d;\n",
...
bool EvictFreeAccounts(void){
	...
	for(int EvictionNr = 0; EvictionNr < NumberOfEvictions; EvictionNr += 1){
		...
		Log("houses", "Account for house %d is not longer paid for.\n", HouseID);
...
bool EvictExGuildLeaders(void){
	...
	if(NumberOfGuildHouses > 0){
		...
		for(int EvictionNr = 0; EvictionNr < NumberOfEvictions; EvictionNr += 1){
			...
			Log("houses", "Owner of guildhall %d is no longer guild leader.\n", HouseID);
			CleanHouse(House, NULL);
			ClearHouse(House);
			House->Help = 1;
		}
...
void CollectRent(void){
	...
	for(int HouseNr = 0; HouseNr < Houses; HouseNr += 1){
		...
		if(PlayerData == NULL){
			error("CollectRents: Cannot assign slot for player data (2).\n");
			continue;
		}
		...
		if(DepotMoney < H->Rent){
			if(TimeStamp < Deadline){
				Log("houses", "Owner receives reminder.\n");
				...
				Log("houses", "Owner is being thrown out.\n");

				if(QueryManagerConnection->excludeFromAuctions(H->OwnerID, false) != 0){
					error("CollectRents: Cannot ban tenant.\n");
				}
			...
			Log("houses", "Owner has enough money.\n");
...
bool StartAuctions(void){
	...
	if(Ret != 0){
		error("StartAuctions: Cannot determine running auctions.\n");
		return false;
	}
	...
	for(int HouseNr = 0; HouseNr < Houses; HouseNr += 1){
		...
		if(H->OwnerID == 0 && H->Help == 0 && !H->NoAuction){
			Log("houses", "Entering house %d for auction.\n", H->ID);
			Log("houses", "At reset: DELETE FROM Auctions WHERE World=\'%s\' AND HouseID=%d;\n",
					HelpWorld, H->ID);
...
bool UpdateHouseOwners(void){
	...
	Log("houses", "Updating list of owners...\n");
	for(int HouseNr = 0; HouseNr < Houses; HouseNr += 1){
		THouse *H = House.at(HouseNr);
		...
		if(H->OwnerID != 0 && H->Help == 0){
			Log("houses", "Entering rented house %d.\n", H->ID);
		...
		if(H->Help == 2){
			...
			if(QueryManagerConnection->updateHouseOwner(H->ID, H->OwnerID, H->PaidUntil) != 0){
				error("StartAuctions: Cannot update data of rented house %d.\n", H->ID);
			}
		}
	}
...
void FinishHouseCleanup(void){
	for(int HelpDepotNr = 0; HelpDepotNr < HelpDepots; HelpDepotNr += 1){
		...
		if(PlayerData == NULL){
			error("FinishHouseCleanup: Cannot assign slot for player data.\n");
			continue;
		}
...
void CleanHouseField(int x, int y, int z){
	...
	if(HelpDepotNr >= HelpDepots){
		...
		if(PlayerData == NULL){
			error("CleanHouseField: Cannot assign slot for player data.\n");
			return;
		}
...
void LoadHouseAreas(void){
	Log("houses", "Loading data of house areas...\n");

	char FileName[4096];
	snprintf(FileName, sizeof(FileName), "%s/houseareas.dat", DATAPATH);
	if(!FileExists(FileName)){
		Log("houses", "No data for house areas gound.\n");
		return;
	}
...
void LoadHouses(void){
	...
	while(true){
		...
		if(Houses > 0 && HouseID <= House.at(Houses - 1)->ID){
			error("LoadHouses: IDs not sorted ascending (ID=%d).\n", HouseID);
			throw "cannot load houses";
		}
	...
	if(Ret != 0){
		error("LoadHouses: Cannot register basic data of houses.\n");
	}
}

void LoadOwners(void){
	Log("houses", "Loading owner data...\n");
	...
	if(!FileExists(FileName)){
		Log("houses", "No owner data found.\n");
		return;
	}
			...
			for(int FieldX = MinX; FieldX <= MaxX; FieldX += 1){
				...
				if(Bed != NONE && Bed.getAttribute(TEXTSTRING) != 0){
					try{
						...
						error("LoadOwners: Exception %d while cleaning a bed.\n", r);
					}
				}
			}
		}
