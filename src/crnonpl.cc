...
int TBehaviourDatabase::evaluate(TNPC *Npc, TBehaviourNode *Node, int *Parameters){
	...
	if(Parameters == NULL){
		error("TBehaviourDatabase::evaluate: Numbers do not exist.\n");
		return 0;
	}
	...
	switch(Node->Type){
		...
		case BEHAVIOUR_NODE_PARAMETER:{
			if(Node->Data != 1 && Node->Data != 2){
				...
				error("TBehaviourDatabase::evaluate: Number parameter %d not set.\n", Node->Data);
			}else{
...
void TBehaviourDatabase::react(TNPC *Npc, const char *Text, SITUATION Situation){
	...
	if(Text == NULL){
		error("TBehaviourDatabase::react: Passed text does not exist.\n");
		return;
	}
			...
			switch(Action->Type){
				case BEHAVIOUR_ACTION_REPLY:{
					...
					if(FormatNpcResponse(Response, sizeof(Response), Template, Npc, Interlocutor)){
						...
						Response[20] = 0;
						error("TBehaviourDatabase::react: Text from NPC %s becomes too long (%s...).\n",
								Npc->Name, Response);
					}
					break;
				}
				...
				case BEHAVIOUR_ACTION_FUNCTION3:{
					...
					switch(FunctionNr){
						case 1:{
							print(3, "NPC teleports conversation partner to [%d,%d,%d].\n",
						...
						case 2:{
							print(3, "NPC sets start coordinate for conversation partner to [%d,%d,%d].\n",
									Param1, Param2, Param3);
						...
						default:{
							error("TBehaviourDatabase::react (10): Invalid subnumber.\n");
							break;
						}
					}
					break;
...
void StartMonsterhomeTimer(int Nr){
	...
	if(MH->Timer > 0){
		error("StartMonsterhomeTimer: Counter is already running.\n");
		return;
	}
...
void LoadMonsterhomes(void){
	...
	while(true){
		...
		if(!IsOnMap(MH->x, MH->y, MH->z)){
			print(1, "WARNING: Monsterhome [%d,%d,%d] located outside the map.\n", MH->x, MH->y, MH->z);
		}
	}
		...
		QueuedAddresses(0, 9, 10)
{
	...
	if(!IsOnMap(this->posx, this->posy, this->posz)){
		print(1, "WARNUNG: NPC '%s' is located outside the map.\n", this->Name);
		return;
	}
...
void TNPC::TalkStimulus(uint32 SpeakerID, const char *Text){
	...
	if(Text == NULL){
		error("TNPC::TalkStimulus: Passed text does not exist.\n");
		return;
	}
		...
		TNonplayer()
{
	...
	try{
			...
			for(int i = 1; i <= RaceData[Race].Items; i += 1){
				...
				for(int j = 0; j < Repeat; j += 1){
					...
					try{
						...
						error("TMonster::TMonster: Exception %d at race %d, ggf."
					...
					if(Item.getContainer().getObjectType().isMapContainer()){
						error("TMonster::TMonster: Object falls onto the map."
		...
		error("TMonster::TMonster: Exception %d at race %d.\n", r, Race);
	}
...
bool TMonster::MovePossible(int x, int y, int z, bool Execute, bool Jump){
	if(this->posz != z){
		error("TMonster::MovePossible: Check across levels ([%d,%d,%d] -> [%d,%d,%d]).\n",
	...
	if(this->Skills[SKILL_GO_STRENGTH]->Act < 0){
		error("TMonster::MovePossible: Monster %s at [%d,%d,%d] may not move.\n",
	...
	for(int Attempt = 0; Attempt < 100; Attempt += 1){
		...
		while(Obj != NONE){
			...
			if(ObjType.isCreatureContainer()){
				...
				if(Creature == NULL){
					error("TMonster::MovePossible: Cannot identify obstacle creature.\n");
					return false;
				}
	...
	if(this->Master != 0){
		...
		if(Master == NULL){
			if(MasterIsPlayer){
				...
				print(3, "Creature %s loses its mother monster.\n", this->Name);
			}
			ShouldDespawn = true;
			...
			print(3, "Mother monster is too far away.\n");
			ShouldDespawn = true;
		...
		if(!MonsterhomeInRange(this->Home, this->posx, this->posy, this->posz)){
			...
			print(3, "%s at [%d,%d,%d] is too far from [%d,%d,%d].\n",
	...
	if(RaceData[this->Race].Talks > 0 && (rand() % 50) == 0){
		...
		if(Text != 0 && Text[0] != 0){
			...
			try{
				...
				error("TMonster::IdleStimulus: Exception %d at talk.\n", r);
			}
		}else{
...
void TMonster::KickBoxes(Object Obj){
	if(!Obj.exists()){
		error("TMonster::KickBoxes: Passed object does not exist.\n");
		return;
	}
...
bool TMonster::KickCreature(TCreature *Creature){
	if(Creature == NULL){
		error("TMonster::KickCreature: Passed creature does not exist.\n");
		return false;
	}

	if(Creature->Type != MONSTER){
		error("TMonster::KickCreature: Creature to be moved is not a monster.\n");
		return false;
	}
	...
	try{
		...
		if(!CreatureMoved){
			print(3, "No psace to move => Killing.\n");
		...
		error("# Objects on target field [%d,%d,%d]:\n", DestX, DestY, DestZ);
