...
const char *GetLiquidName(int LiquidType){
	...
	switch(LiquidType) {
		...
		default:{
			error("GetLiquidName: Invalid fluid type %d\n", LiquidType);
			LiquidName = "unknown";
			break;
		}
	}
...
uint8 GetLiquidColor(int LiquidType){
	...
	switch(LiquidType){
		...
		default:{
			error("GetLiquidColor: Invalid fluid type %d\n", LiquidType);
...
int GetObjectRNum(Object Obj){
	...
	if(ObjType.isMapContainer()){
		error("GetObjectRNum: Object is a MapContainer.\n");
		return 0;
	}
...
void CalculateChange(int Amount, int *Gold, int *Platinum, int *Crystal){
	...
	if((Cr * 10000 + Pl * 100 + Go) < Amount){
		error("CalculateChange: %d/%d/%d coins are not enough to pay for %d.\n",
				Go, Pl, Cr, Amount);
		return;
	}
	...
	*Crystal = Cr;
	...
	if((Cr * 10000 + Pl * 100 + Go) != Amount){
		error("CalculateChange: Faulty calculation: %d/%d/%d coins for %d.\n",
				Go, Pl, Cr, Amount);
	}
}
