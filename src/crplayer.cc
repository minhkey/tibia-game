		...
		FormerAttackedPlayers(0, 10, 10)
{
	...
	if(!CheckRight(CharacterID, PREMIUM_ACCOUNT)){
		if(IsPremiumArea(this->startx, this->starty, this->startz)){
			Log("game", "Player %s is exiled from PayArea city and receives new hometown.\n", this->Name);
		...
		if(IsPremiumArea(this->posx, this->posy, this->posz)){
			Log("game", "Player %s is thrown out of PayArea and placed in their hometown.\n", this->Name);
			this->posx = this->startx;
			this->posy = this->starty;
			this->posz = this->startz;
		}
...
TPlayer::~TPlayer(void){
	...
	if(!this->IsDead){
		...
		if(this->Profession != PROFESSION_NONE && this->Skills[SKILL_LEVEL]->Get() <= 5){
			Log("game", "Resetting player %s completely due to level.\n", this->Name);
			ResetCharacter = true;
		}else if(this->Skills[SKILL_HITPOINTS]->Max <= 0){
			Log("game", "Resetting player %s completely due to MaxHitpoints.\n", this->Name);
			ResetCharacter = true;
		}
...
void TPlayer::Death(void){
	if(CheckRight(this->ID, INVULNERABLE)){
		error("TPlayer::Death: Aha, that doesn't work like that!! You cannot kill gods!!\n");
		this->Skills[SKILL_HITPOINTS]->SetMax();
		return;
	}
...
void TPlayer::LoadInventory(bool SetStandardInventory){
	...
	if(PlayerData->Inventory != NULL){
		...
		try{
			...
			error("TPlayer::LoadInventory: Exception %d while creating standard inventory.\n", r);
		}
	}
}
...
void TPlayer::SetProfession(uint8 Profession){
	if(Profession == PROFESSION_PROMOTION){
		if(this->Profession == PROFESSION_NONE){
			error("TPlayer::SetProfession: Player has no profession for promotion yet.\n");
			return;
		}

		if(this->Profession >= PROFESSION_PROMOTION){
			error("TPlayer::SetProfession: Player has already promoted their profession.\n");
			return;
		}
...
void TPlayer::LearnSpell(int SpellNr){
	...
	if(this->SpellList[SpellNr] != 0){
		error("TPlayer::LearnSpell: The player already knows the spell.\n");
		return;
	}
...
void TPlayer::SetQuestValue(int QuestNr, int Value){
	...
	print(3, "New value for quest variable %d from %s: %d.\n",
...
void TPlayer::Regenerate(void){
	...
	if(!Bed.getObjectType().getFlag(TEXT)){
		error("TPlayer::Regenerate: Bed carries no text.\n");
		return;
	}
	...
	try{
		...
		error("TPlayer::Regenerate: Exception %d while cleaning the bed.\n", r);
	}
}
...
void TPlayer::RecordAttack(uint32 VictimID){
	...
	if(!Victim->InPartyWith(this, true)
		...
		*this->AttackedPlayers.at(this->NumberOfAttackedPlayers) = VictimID;
		...
		print(3, "Player %s is attacker for player %s.\n", this->Name, Victim->Name);
...
void TPlayer::RecordMurder(uint32 VictimID){
	...
	// Need to switch victim and murderer since English grammar is different
	print(3, "Unjustified murder of %s by %s.\n", Victim->Name, this->Name);
	...
	if(Playerkilling != 0){
		...
		if(OldPlayerkillerEnd == 0){
			print(3, "Player %s is a player killer.\n", this->Name);
			AnnounceChangedCreature(this->ID, CREATURE_SKULL_CHANGED);
		}
...
void TPlayer::ClearPlayerkillingMarks(void){
	print(3, "Clearing marks for player %s.\n", this->Name);
	...
	if(this->Aggressor){
		...
		for(int i = 0; i < this->NumberOfFormerAttackedPlayers; i += 1){
			...
			if(Victim != NULL){
				print(3, "Player %s is no longer an attacker for player %s.\n", this->Name, Victim->Name);
				if(Victim->Connection != NULL
				&& Victim->Connection->KnownCreature(this->ID, false) == KNOWNCREATURE_UPTODATE){
					SendCreatureSkull(Victim->Connection, this->ID);
				}
...
void LogoutAllPlayers(void){
	print(1, "LogoutAllPlayers: Will log out all players!\n");
	for(int Index = 0; Index < FirstFreePlayer; Index += 1){
		TPlayer *Player = *PlayerList.at(Index);
		...
		if(Player == NULL){
			error("LogoutAllPlayers: Entry %d in player list is NULL.\n", Index);
			continue;
		}
...
bool LoadPlayerData(TPlayerData *Slot){
	...
	if(Slot->CharacterID == 0){
		error("LoadPlayerData: Slot contains no character.\n");
		return false;
	}
	...
	try{
		...
		error("LoadPlayerData: No memory free while loading player %u.\n",
				Slot->CharacterID);
	}
...
void SavePlayerData(TPlayerData *Slot){
	...
	if(Slot->CharacterID == 0){
		error("SavePlayerData: Slot contains no character.\n");
		return;
	}
...
void FreePlayerPoolSlot(TPlayerData *Slot){
	...
	if(Slot->Sticky > 0){
		error("FreePlayerPoolSlot: Slot is still needed; may not be released.\n");
		return;
	}
...
TPlayerData *GetPlayerPoolSlot(uint32 CharacterID){
	if(CharacterID == 0){
		error("GetPlayerPoolSlot: CharacterID is null.\n");
		return NULL;
	}
...
TPlayerData *AssignPlayerPoolSlot(uint32 CharacterID, bool DontWait){
	if(CharacterID == 0){
		error("AssignPlayerPoolSlot: CharacterID is null.\n");
		return NULL;
	}
...
TPlayerData *AttachPlayerPoolSlot(uint32 CharacterID, bool DontWait){
	if(CharacterID == 0){
		error("AttachPlayerPoolSlot: CharacterID is null.\n");
		return NULL;
	}

	print(3, "Attaching slot from player %u.\n", CharacterID);
...
void DecreasePlayerPoolSlotSticky(uint32 CharacterID){
	if(CharacterID == 0){
		error("DecreasePlayerPoolSlotSticky: CharacterID is null.\n");
		return;
	}
...
void InitPlayerIndex(void){
	...
	while(true){
		...
		if(Ret != 0){
			error("InitPlayerIndex: Cannot determine player data.\n");
			break;
		}
