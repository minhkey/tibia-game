...
uint32 Object::getAttribute(INSTANCEATTRIBUTE Attribute){
	...
	if(AttributeOffset == -1){
		error("Object::getAttribute: Flag for attribute %d at object type %d not set.\n",
				Attribute, ObjType.TypeID);
		return 0;
	}

	if(AttributeOffset < 0 || AttributeOffset >= NARRAY(TObject::Attributes)){
		error("Object::getAttribute: Invalid offset %d for attribute %d at object type %d.\n",
...
void Object::setAttribute(INSTANCEATTRIBUTE Attribute, uint32 Value){
	...
	int AttributeOffset = ObjType.getAttributeOffset(Attribute);
	if(AttributeOffset == -1){
		error("Object::setAttribute: Flag for attribute %d at object type %d not set.\n",
				Attribute, ObjType.TypeID);
		return;
	}

	if(AttributeOffset < 0 || AttributeOffset >= NARRAY(TObject::Attributes)){
		error("Object::setAttribute: Invalid offset %d for attribute %d at object type %d.\n",
				AttributeOffset, Attribute, ObjType.TypeID);
		return;
	}
...
static void CronSet(Object Obj, uint32 Delay){
	if(!Obj.exists()){
		error("CronSet: Passed object does not exist.\n");
		return;
	}
...
void CronExpire(Object Obj, int Delay){
	if(!Obj.exists()){
		error("CronExpire: Passed object does not exist.\n");
		return;
	}
...
void CronChange(Object Obj, int NewDelay){
	if(!Obj.exists()){
		error("CronChange: Passed object does not exist.\n");
		return;
	}
...
uint32 CronInfo(Object Obj, bool Delete){
	if(!Obj.exists()){
		error("CronInfo: Passed object does not exist.\n");
		return 0;
	}
...
uint32 CronStop(Object Obj){
	if(!Obj.exists()){
		error("CronStop: Passed object does not exist.\n");
		return 0;
	}
...
static void ResizeHashTable(void){
	...
	error("INFO: Hash table too small. Size doubled to %d.\n", NewSize);
	...
	for(uint32 i = 1; i < NewSize; i += 1){
		if(HashTableType[i] != STATUS_FREE){
			...
			if(HashTableType[i] == STATUS_LOADED){
				...
				error("ResizeHashTable: Error while reorganizing hash table.\n");
			}
		}
	}
...
static TObject *GetFreeObjectSlot(void){
	...
	if(Entry == NULL){
		error("GetFreeObjectSlot: No free space left.\n");
		return NULL;
	}
...
void SwapObject(TWriteBinaryFile *File, Object Obj, uintptr FileNumber){
	...
	if(Entry->ObjectID != Obj.ObjectID){
		error("SwapObject: Passed object does not exist.\n");
		return;
	}
...
void SwapSector(void){
	...
	if(Oldest == NULL){
		error("FATAL ERROR in SwapSector: Sector cannot be swapped out.\n");
		abort();
	}
	...
	try{
		...
		error("FATAL ERROR in SwapSector: Cannot file \"%s\" nicht anlegen.\n", FileName);
		error("# Error: %s\n", str);
		abort();
	}
}
...
void PatchSector(int SectorX, int SectorY, int SectorZ, bool FullSector,
	...
	if(NewSector){
		print(2, "Creating sector %d/%d/%d anew.\n", SectorX, SectorY, SectorZ);
		InitSector(SectorX, SectorY, SectorZ);
	...
	if(rename(FileNameBak, FileName) != 0){
		int ErrCode = errno;
		error("PatchSector: Error %d while renaming %s.\n", ErrCode, FileNameBak);
...
static void DestroyObject(Object Obj){
	if(!Obj.exists()){
		error("DestroyObject: Passed object does not exist.\n");
		return;
	}
...
void DeleteObject(Object Obj){
	if(!Obj.exists()){
		error("DeleteObject: Passed object does not exist.\n");
		return;
	}
...
void ChangeObject(Object Obj, ObjectType NewType){
	if(!Obj.exists()){
		error("ChangeObject: Passed object does not exist.\n");
		return;
	}
...
int GetObjectPriority(Object Obj){
	if(!Obj.exists()){
		error("GetObjectPriority: Passed object does not exist.\n");
		return -1;
	}
...
void PlaceObject(Object Obj, Object Con, bool Append){
	if(!Obj.exists()){
		error("PlaceObject: Passed object does not exist.\n");
		return;
	}

	if(!Con.exists()){
		error("PlaceObject: Passed container does not exist.\n");
		return;
	}
...
void CutObject(Object Obj){
	if(!Obj.exists()){
		error("CutObject: Passed object does not exist.\n");
		return;
	}
...
void MoveObject(Object Obj, Object Con){
	if(!Obj.exists()){
		error("MoveObject: Passed object does not exist.\n");
		return;
	}

	if(!Con.exists()){
		error("MoveObject: Passed target container does not exist.\n");
		return;
	}
...
Object AppendObject(Object Con, ObjectType Type){
	if(!Con.exists()){
		error("AppendObject: Passed container does not exist.\n");
		return NONE;
	}
...
Object SetObject(Object Con, ObjectType Type, uint32 CreatureID){
	if(!Con.exists()){
		error("SetObject: Passed container does not exist.\n");
		return NONE;
	}
...
Object CopyObject(Object Con, Object Source){
	if(!Source.exists()){
		error("CopyObject: Template does not exist.\n");
		return NONE;
	}

	if(!Con.exists()){
		error("CopyObject: Target container does not exist.\n");
		return NONE;
	}

	ObjectType SourceType = Source.getObjectType();
	if(SourceType.isCreatureContainer()){
		error("CopyObject: Creatures may not be copied.\n");
		return NONE;
	}
...
Object SplitObject(Object Obj, int Count){
	if(!Obj.exists()){
		error("SplitObject: Passed object does not exist.\n");
		return NONE;
	}
	...
	if(Count <= 0 || (uint32)Count > Amount){
		error("SplitObject: Invalid counter %d.\n", Count);
		return Obj; // TODO(fusion): Probably return NONE?
	}
...
void MergeObjects(Object Obj, Object Dest){
	if(!Obj.exists()){
		error("MergeObjects: Passed object does not exist.\n");
		return;
	}

	if(!Dest.exists()){
		error("MergeObjects: Passed target does not exist.\n");
		return;
	}
	...
	if(DestAmount == 0){
		error("MergeObjects: Target object contains 0 parts.\n");
		DestAmount = 1;
	}
...
Object GetFirstContainerObject(Object Con){
	if(Con == NONE){
		error("GetFirstContainerObject: Passed container does not exist.\n");
		return NONE;
	}
...
Object GetContainerObject(Object Con, int Index){
	if(Index < 0){
		error("GetContainerObject: Invalid running number %d.\n", Index);
		return NONE;
	}

	if(!Con.exists()){
		error("GetContainerObject: Passed container does not exist.\n");
		return NONE;
	}
...
Object GetMapContainer(Object Obj){
	if(!Obj.exists()){
		error("GetMapContainer: Passed object does not exist\n");
		return NONE;
	}
...
void GetObjectCoordinates(Object Obj, int *x, int *y, int *z){
	if(!Obj.exists()){
		error("GetObjectCoordinates: Passed object does not exist.\n");
		*x = 0;
		*y = 0;
		*z = 0;
		return;
