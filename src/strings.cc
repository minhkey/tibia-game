...
uint32 AddDynamicString(const char *String){
	...
	if(EntryIndex == -1){
		error("AddDynamicString: No free space found.\n");
		return 0;
	}
		...
		for(int i = 0; i < DynamicBlockEntries; i += 1){
			if(Block->EntryType[i] == DYNAMIC_STRING_DELETED){
				...
				if(StringEnd > DynamicBlockSize){
					error("CleanupDynamicStrings: String end missing\n");
					continue;
				}
...
const char *Plural(const char *s, int Count){
	...
	if(s == NULL){
		error("Plural: Passed string does not exist.\n");
...
const char *SearchForWord(const char *Pattern, const char *Text){
	if(Pattern == NULL || Pattern[0] == 0){
		error("SearchForWord: Passed search word does not exist.\n");
		return NULL;
	}

	if(Text == NULL){
		error("SearchForWord: Passed text does not exist.\n");
		return NULL;
	}
...
const char *SearchForNumber(int Count, const char *Text){
	...
	if(Text == NULL){
		error("SearchForNumber: Passed text does not exist.\n");
		return NULL;
	}
