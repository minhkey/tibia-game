...
static const char SpellSyllable[51][6] = {
    "",    "al",   "ad",   "ex",    "ut",   "om",   "para",  "ana",  "evo",
    "ori", "mort", "lux",  "liber", "vita", "flam", "pox",   "hur",  "moe",
    "ani", "ina",  "eta",  "amo",   "hora", "gran", "cogni", "res",  "mas",
    "vis", "som",  "aqua", "frigo", "tera", "ura",  "sio",   "grav", "ito",
    "pan", "vid",  "isa",  "iva",   "con",  "",     "",      "",     "",
    "",    "",     "",     "",      "",     "",
};
...
bool TImpact::isAggressive(void) { return true; }

// TDamageImpact
// =============================================================================
TDamageImpact::TDamageImpact(TCreature *Actor, int DamageType, int Power,
                             bool AllowDefense) {
  if (Actor == NULL) {
...
bool THealingImpact::isAggressive(void) { return false; }
...
// TStrengthImpact
// =============================================================================
TStrengthImpact::TStrengthImpact(TCreature *Actor, int Skills, int Percent,
                                 int Duration) {
  if (Actor == NULL) {
...
void TSummonImpact::handleField(int x, int y, int z) {
  ...
  if (Actor != NULL && IsRaceValid(Race) &&
...
void VictimShapeSpell(TCreature *Actor, TCreature *Victim, int Range,
                      int Animation, TImpact *Impact, int Effect) {
  if (Actor == NULL) {
  ...
  int Distance = std::max<int>(std::abs(Actor->posx - Victim->posx),
  ...
  if (Impact->isAggressive() &&
      IsProtectionZone(Victim->posx, Victim->posy, Victim->posz)) {
    return;
  }

  if (!ThrowPossible(Actor->posx, Actor->posy, Actor->posz, Victim->posx,
                     Victim->posy, Victim->posz, 0)) {
    return;
  }
...
// inlined there.
static void ExecuteCircleSpell(int DestX, int DestY, int DestZ, int Radius,
...
void OriginShapeSpell(TCreature *Actor, int Radius, TImpact *Impact,
                      int Effect) {
  if (Actor == NULL) {
  ...
  ExecuteCircleSpell(Actor->posx, Actor->posy, Actor->posz, Radius, Impact,
                     Effect);
}

void CircleShapeSpell(TCreature *Actor, int DestX, int DestY, int DestZ,
                      int Range, int Animation, int Radius, TImpact *Impact,
                      int Effect) {
  if (Actor == NULL) {
  ...
  int Distance = std::max<int>(std::abs(Actor->posx - DestX),
                               std::abs(Actor->posy - DestY));
  if (Distance > Range || Actor->posz != DestZ) {
    return;
  }

  if (!ThrowPossible(Actor->posx, Actor->posy, Actor->posz, DestX, DestY, DestZ,
                     0)) {
    return;
  }
...
void DestinationShapeSpell(TCreature *Actor, TCreature *Victim, int Range,
                           int Animation, int Radius, TImpact *Impact,
                           int Effect) {
  if (Actor == NULL) {
  ...
  if (Victim != NULL) {
    CircleShapeSpell(Actor, Victim->posx, Victim->posy, Victim->posz, Range,
                     Animation, Radius, Impact, Effect);
  }
}

void AngleShapeSpell(TCreature *Actor, int Angle, int Range, TImpact *Impact,
                     int Effect) {
  if (Actor == NULL) {
...
void CheckRuneLevel(TCreature *Actor, int SpellNr) {
  ...
  if (Actor->Type == PLAYER && !CheckRight(Actor->ID, ALL_SPELLS)) {
    ...
    if (MagicLevel == NULL) {
      error("CheckRuneLevel: No skill MAGLEVEL.\n");
      throw ERROR;
    }
...
void CheckMagicItem(TCreature *Actor, ObjectType Type) {
  if (Actor == NULL) {
    error("CheckMagicItem: Passed creature does not exist.\n");
    throw ERROR;
  }
...
void CheckAffectedPlayers(TCreature *Actor, int x, int y, int z) {
  ...
  if (WorldType == NORMAL && Actor->Type == PLAYER &&
...
void MassCombat(TCreature *Actor, Object Target, int ManaPoints, int SoulPoints,
                int Damage, int Effect, int Radius, int DamageType,
                int Animation) {
  if (!Target.exists()) {
  ...
  if (!ThrowPossible(Actor->posx, Actor->posy, Actor->posz, TargetX, TargetY,
                     TargetZ, 0)) {
    throw CANNOTTHROW;
  }
  ...
  CircleShapeSpell(Actor, TargetX, TargetY, TargetZ, INT_MAX, Animation, Radius,
                   &Impact, Effect);
}

void AngleCombat(TCreature *Actor, int ManaPoints, int SoulPoints, int Damage,
                 int Effect, int Range, int Angle, int DamageType) {
  if (Actor == NULL) {
...
void Combat(TCreature *Actor, Object Target, int ManaPoints, int SoulPoints,
  ...
  if (!Target.getObjectType().isCreatureContainer()) {
    Target = GetFirstSpecObject(TargetX, TargetY, TargetZ,
                                TYPEID_CREATURE_CONTAINER);
  }
  ...
  TDamageImpact Impact(Actor, DamageType, Damage, false);
  CircleShapeSpell(Actor, TargetX, TargetY, TargetZ, INT_MAX, Animation, 0,
                   &Impact, Effect);
}
...
int GetDirection(int dx, int dy) {
  ...
  if (dx == 0) {
    ...
    // floating point calculations. The tangent is unique and odd in the
    // interval
    // (-PI/2, +PI/2) but since that only covers half the unit circle, we need
    // to mirror results to the other half by comparing the sign of `dx`. The
    // Y-axis is also inverted in Tibia, so we need to negate `dy`.
...
void KillAllMonsters(TCreature *Actor, int Effect, int Radius) {
  ...
  for (int R = 0; R <= Radius; R += 1) {
    ...
    for (int Point = 0; Point < CirclePoints; Point += 1) {
      ...
      if (!ThrowPossible(CenterX, CenterY, CenterZ, FieldX, FieldY, FieldZ,
                         0)) {
        continue;
      }
...
void CreateField(int x, int y, int z, int FieldType, uint32 Owner,
                 bool Peaceful) {
  ...
  try {
    Create(GetMapContainer(x, y, z), GetSpecialObject(Meaning), Owner);
  } catch (RESULT r) {
    if (r != DESTROYED) {
      throw;
    }
...
void CreateField(TCreature *Actor, Object Target, int ManaPoints,
                 int SoulPoints, int FieldType) {
  if (Actor == NULL) {
  ...
  if (FieldType == FIELD_TYPE_FIRE || FieldType == FIELD_TYPE_POISON ||
      FieldType == FIELD_TYPE_ENERGY) {
    Actor->BlockLogout(60, true);
  }
}

void CreateField(TCreature *Actor, int ManaPoints, int SoulPoints,
                 int FieldType) {
  if (Actor == NULL) {
    error("CreateField: Invalid creature passed.\n");
    throw ERROR;
...
void MassCreateField(TCreature *Actor, Object Target, int ManaPoints,
                     int SoulPoints, int FieldType, int Radius) {
  if (Actor == NULL) {
  ...
  if (Actor->posx != TargetX || Actor->posy != TargetY ||
      Actor->posz != TargetZ) {
    int Animation = ANIMATION_ENERGY;
    if (FieldType == FIELD_TYPE_FIRE) {
      Animation = ANIMATION_FIRE;
  ...
  for (int R = 0; R <= Radius; R += 1) {
    ...
    for (int Point = 0; Point < CirclePoints; Point += 1) {
      ...
      if (!ThrowPossible(TargetX, TargetY, TargetZ, FieldX, FieldY, FieldZ,
                         0)) {
        continue;
      }
...
void CreateFieldWall(TCreature *Actor, Object Target, int ManaPoints,
                     int SoulPoints, int FieldType, int Width) {
  if (Actor == NULL) {
...
void DeleteField(TCreature *Actor, Object Target, int ManaPoints,
                 int SoulPoints) {
  if (Actor == NULL) {
...
void CleanupField(TCreature *Actor, Object Target, int ManaPoints,
                  int SoulPoints) {
  if (Actor == NULL) {
  ...
  int Distance = std::max<int>(std::abs(Actor->posx - TargetX),
...
void CleanupField(TCreature *Actor) {
  ...
  if (Actor->Type != PLAYER) {
    error("CleanupField: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void Teleport(TCreature *Actor, const char *Param) {
  ...
  if (Param == NULL) {
    error("Teleport: Parameter is NULL.\n");
    throw ERROR;
  }

  if (Actor->Type != PLAYER) {
    error("Teleport: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void TeleportToCreature(TCreature *Actor, const char *Name) {
  ...
  if (Actor->Type != PLAYER) {
    error("TeleportToCreature: Magic spell can only be used by players.\n");
    throw ERROR;
  }
  ...
  case -2:
    throw NAMEAMBIGUOUS;
  default: {
    error("TeleportToCreature: Invalid return value by IdentifyPlayer.\n");
    throw ERROR;
  }
  }
...
void TeleportPlayerToMe(TCreature *Actor, const char *Name) {
  ...
  if (Actor->Type != PLAYER) {
    error("TeleportPlayerToMe: Magic spell can only be used by players.\n");
    throw ERROR;
  }
  ...
  case -2:
    throw NAMEAMBIGUOUS;
  default: {
    error("TeleportPlayerToMe: Invalid return value by IdentifyPlayer.\n");
    throw ERROR;
  }
  }
...
void MagicClimbing(TCreature *Actor, int ManaPoints, int SoulPoints,
                   const char *Param) {
  if (Actor == NULL) {
...
void MagicClimbing(TCreature *Actor, const char *Param) {
  ...
  if (Actor->Type != PLAYER) {
    error("MagicClimbing: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void CreateThing(TCreature *Actor, const char *Param1, const char *Param2) {
  ...
  if (Actor->Type != PLAYER) {
    error(
        "CreateThing: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void CreateMoney(TCreature *Actor, const char *Param) {
  ...
  if (Actor->Type != PLAYER) {
    error(
        "CreateMoney: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void CreateArrows(TCreature *Actor, int ManaPoints, int SoulPoints,
                  int ArrowType, int Count) {
  if (Actor == NULL) {
...
void SummonCreature(TCreature *Actor, int ManaPoints, int Race, bool God) {
  ...
  if (!IsRaceValid(Race)) {
    error("SummonCreature: Invalid Racenumber %d passed.\n", Race);
    throw ERROR;
  }
...
void SummonCreature(TCreature *Actor, int ManaPoints, const char *RaceName,
                    bool God) {
  if (Actor == NULL) {
    error("SummonCreature: Invalid creature passed.\n");
    throw ERROR;
  ...
  if (RaceName == NULL) {
    error("SummonCreature: Invalid Racenumber passed.\n");
    throw ERROR;
  }
...
void StartMonsterraid(TCreature *Actor, const char *RaidName) {
  ...
  if (RaidName == NULL) {
    error("StartMonsterraid: Invalid Raidname passed.\n");
    throw ERROR;
  }
...
void RaiseDead(TCreature *Actor, Object Target, int ManaPoints,
               int SoulPoints) {
  if (Actor == NULL) {
...
void MassRaiseDead(TCreature *Actor, Object Target, int ManaPoints,
                   int SoulPoints, int Radius) {
  if (Actor == NULL) {
...
void MassHeal(TCreature *Actor, Object Target, int ManaPoints, int SoulPoints,
...
void HealFriend(TCreature *Actor, const char *TargetName, int ManaPoints,
                int SoulPoints, int Amount) {
  if (Actor == NULL) {
...
void MagicGoStrength(TCreature *Actor, TCreature *Target, int ManaPoints,
                     int SoulPoints, int Percent, int Duration) {
  if (Actor == NULL) {
...
void NegatePoison(TCreature *Actor, TCreature *Target, int ManaPoints,
                  int SoulPoints) {
  if (Actor == NULL) {
...
void Enlight(TCreature *Actor, int ManaPoints, int SoulPoints, int Radius,
             int Duration) {
  if (Actor == NULL) {
  ...
  if (Actor->Skills[SKILL_LIGHT]->TimerValue() <= Radius) {
    Actor->SetTimer(SKILL_LIGHT, Radius, Duration / Radius, Duration / Radius,
                    -1);
  }
  GraphicalEffect(Actor->posx, Actor->posy, Actor->posz, EFFECT_MAGIC_BLUE);
}

void Invisibility(TCreature *Actor, int ManaPoints, int SoulPoints,
                  int Duration) {
  if (Actor == NULL) {
        ...
        if (ObjType.isCreatureContainer()) {
          ...
          if (Victim == NULL) {
            ...
            if (Victim->Skills[SKILL_ILLUSION]->Get() == 0) {
              ...
              // must come from an item and it seems there is a chance to
              // destroy it on pvp enforced worlds.
              // TODO(fusion): This is probably an inlined function.
              if (WorldType == PVP_ENFORCED) {
                for (int Position = INVENTORY_FIRST; Position <= INVENTORY_LAST;
                     Position += 1) {
                  ...
                  if (ItemType.getFlag(SKILLBOOST) &&
                      ...
                      (int)ItemType.getAttribute(SKILLNUMBER) ==
                          SKILL_ILLUSION) {
                    if (random(1, 5) == 1) {
                      Delete(Item, -1);
                    }
...
void CreatureIllusion(TCreature *Actor, int ManaPoints, int SoulPoints,
                      const char *RaceName, int Duration) {
  if (Actor == NULL) {
  ...
  if (RaceName == NULL) {
    error("CreatureIllusion: Invalid Racenumber passed.\n");
    throw ERROR;
  }
...
void ObjectIllusion(TCreature *Actor, int ManaPoints, int SoulPoints,
                    Object Target, int Duration) {
  if (Actor == NULL) {
...
void ChangeData(TCreature *Actor, const char *Param) {
  ...
  if (Actor->Type != PLAYER) {
    error(
        "ChangeData: Magic spell can only be used by players.\n");
    throw ERROR;
  }
  ...
  if (Obj != NONE) {
    ...
    if (Attribute != -1) {
      ...
      GraphicalEffect(Actor->posx, Actor->posy, Actor->posz,
                      EFFECT_MAGIC_GREEN);
    } else {
      SendMessage(Actor->Connection, TALK_FAILURE_MESSAGE,
...
void EnchantObject(TCreature *Actor, int ManaPoints, int SoulPoints,
                   ObjectType OldType, ObjectType NewType) {
  if (Actor == NULL) {
...
void Challenge(TCreature *Actor, int ManaPoints, int SoulPoints, int Radius) {
  ...
  for (int R = 0; R <= Radius; R += 1) {
    ...
    for (int Point = 0; Point < CirclePoints; Point += 1) {
      ...
      GraphicalEffect(FieldX, FieldY, FieldZ, EFFECT_MAGIC_BLUE);
      Object Obj =
          GetFirstSpecObject(FieldX, FieldY, FieldZ, TYPEID_CREATURE_CONTAINER);
      if (Obj != NONE) {
        TCreature *Victim = GetCreature(Obj);
        if (Victim == NULL) {
...
void FindPerson(TCreature *Actor, int ManaPoints, int SoulPoints,
  ...
  if (Actor->Type != PLAYER) {
    error(
        "FindPerson: Magic spell can only be used by players.\n");
    throw ERROR;
  }
  ...
  if (Target == NULL) {
    error("FindPerson: Opponent is NULL.\n");
    throw ERROR;
  }

  CheckMana(Actor, ManaPoints, SoulPoints, 1000);

  int Distance = std::max<int>(std::abs(Actor->posx - Target->posx),
                               std::abs(Actor->posy - Target->posy));
  if (Distance <= 4) {
    if (Actor->posz > Target->posz) {
      SendMessage(Actor->Connection, TALK_INFO_MESSAGE, "%s is above you.",
                  Target->Name);
    } else if (Actor->posz < Target->posz) {
      SendMessage(Actor->Connection, TALK_INFO_MESSAGE, "%s is below you.",
                  Target->Name);
    } else {
      SendMessage(Actor->Connection, TALK_INFO_MESSAGE,
                  "%s is standing next to you.", Target->Name);
    }
  } else {
    const char *Direction = "";
    switch (
        GetDirection(Target->posx - Actor->posx, Target->posy - Actor->posy)) {
    case DIRECTION_NORTH:
      Direction = "north";
      break;
    case DIRECTION_EAST:
    ...
    if (Distance <= 99) {
      if (Actor->posz > Target->posz) {
        SendMessage(Actor->Connection, TALK_INFO_MESSAGE,
                    "%s is on a higher level to the %s.", Target->Name,
                    Direction);
      } else if (Actor->posz < Target->posz) {
        SendMessage(Actor->Connection, TALK_INFO_MESSAGE,
                    "%s is on a lower level to the %s.", Target->Name,
                    Direction);
      } else {
        SendMessage(Actor->Connection, TALK_INFO_MESSAGE, "%s is to the %s.",
                    Target->Name, Direction);
      }
    } else if (Distance <= 250) {
      SendMessage(Actor->Connection, TALK_INFO_MESSAGE, "%s is far to the %s.",
                  Target->Name, Direction);
    } else {
      SendMessage(Actor->Connection, TALK_INFO_MESSAGE,
                  "%s is very far to the %s.", Target->Name, Direction);
    }
  }
...
void GetPosition(TCreature *Actor) {
  ...
  if (Actor->Type != PLAYER) {
    error(
        "GetPosition: Magic spell can only be used by players.\n");
    throw ERROR;
  }

  if (CheckRight(Actor->ID, SHOW_COORDINATE)) {
    SendMessage(Actor->Connection, TALK_EVENT_MESSAGE,
                "Your position is [%d,%d,%d].", Actor->posx, Actor->posy,
                Actor->posz);
  }
}
...
void GetQuestValue(TCreature *Actor, const char *Param) {
  ...
  if (Param == NULL) {
    error("GetQuestValue: Parameter is NULL.\n");
    return; // TODO(fusion): Why don't we throw here?
  }

  if (Actor->Type != PLAYER) {
    error("GetQuestValue: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void SetQuestValue(TCreature *Actor, const char *Param1, const char *Param2) {
  ...
  if (Actor->Type != PLAYER) {
    error("SetQuestValue: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void ClearQuestValues(TCreature *Actor) {
  ...
  if (Actor->Type != PLAYER) {
    error("ClearQuestValues: Magic spell can only be used by players.\n");
    throw ERROR;
  }

  if (CheckRight(Actor->ID, CHANGE_SKILLS)) {
    for (int QuestNumber = 0; QuestNumber < NARRAY(TPlayer::QuestValues);
         QuestNumber += 1) {
      ((TPlayer *)Actor)->SetQuestValue(QuestNumber, 0);
    }
    SendMessage(Actor->Connection, TALK_INFO_MESSAGE,
                "All quest values deleted.");
  }
}
...
void CreateKnowledge(TCreature *Actor, const char *Param1, const char *Param2) {
  ...
  if (Actor->Type != PLAYER) {
    error("CreateKnowledge: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void ChangeProfession(TCreature *Actor, const char *Param) {
  ...
  if (Actor->Type != PLAYER) {
    error("ChangeProfession: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void EditGuests(TCreature *Actor) {
  ...
  if (Actor->Type != PLAYER) {
    error(
        "EditGuests: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void EditSubowners(TCreature *Actor) {
  ...
  if (Actor->Type != PLAYER) {
    error("EditSubowners: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void EditNameDoor(TCreature *Actor) {
  ...
  if (Actor->Type != PLAYER) {
    error("EditNameDoor: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void KickGuest(TCreature *Actor, const char *GuestName) {
  ...
  if (GuestName == NULL) {
    error("KickGuest(magic): Invalid gues passed.\n");
    throw ERROR;
  }

  if (Actor->Type != PLAYER) {
    error("KickGuest(magic): Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void Notation(TCreature *Actor, const char *Name, const char *Comment) {
  ...
  if (Comment == NULL) {
    error("Notation: Ungültige Bemerkung passed.\n");
    throw ERROR;
  }

  if (Actor->Type != PLAYER) {
    error("Notation: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void NameLock(TCreature *Actor, const char *Name) {
  ...
  if (Actor->Type != PLAYER) {
    error("NameLock: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void BanishAccount(TCreature *Actor, const char *Name, int Duration,
                   const char *Reason) {
  if (Actor == NULL) {
  ...
  if (Reason == NULL) {
    error("BanishAccount: Invalid Grund passed.\n");
    throw ERROR;
  }

  if (Actor->Type != PLAYER) {
    error("BanishAccount: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void DeleteAccount(TCreature *Actor, const char *Name, const char *Reason) {
  ...
  if (Reason == NULL) {
    error("DeleteAccount: Invalid Grund passed.\n");
    throw ERROR;
  }

  if (Actor->Type != PLAYER) {
    error("DeleteAccount: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void BanishCharacter(TCreature *Actor, const char *Name, int Duration,
                     const char *Reason) {
  if (Actor == NULL) {
  ...
  if (Reason == NULL) {
    error("BanishCharacter: Invalid Grund passed.\n");
    throw ERROR;
  }

  if (Actor->Type != PLAYER) {
    error("BanishCharacter: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void DeleteCharacter(TCreature *Actor, const char *Name, const char *Reason) {
  ...
  if (Reason == NULL) {
    error("DeleteCharacter: Invalid Grund passed.\n");
    throw ERROR;
  }

  if (Actor->Type != PLAYER) {
    error("DeleteCharacter: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void IPBanishment(TCreature *Actor, const char *Name, const char *Reason) {
  ...
  if (Reason == NULL) {
    error("IPBanishment: Invalid Grund passed.\n");
    throw ERROR;
  }

  if (Actor->Type != PLAYER) {
    error("IPBanishment: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void SetNameRule(TCreature *Actor, const char *Name) {
  ...
  if (Actor->Type != PLAYER) {
    error(
        "SetNameRule: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void KickPlayer(TCreature *Actor, const char *Name) {
  ...
  if (Actor->Type != PLAYER) {
    error(
        "KickPlayer: Magic spell can only be used by players.\n");
    throw ERROR;
  }
...
void HomeTeleport(TCreature *Actor, const char *Name) {
  ...
  if (Actor->Type != PLAYER) {
    error("HomeTeleport: Magic spell can only be used by players.\n");
    throw ERROR;
  }
  ...
  Log("banish", "%s teleports %s to temple.\n", Actor->Name, Player->Name);
}

// Spell Casting
// =============================================================================
static void CharacterRightSpell(uint32 CreatureID, int SpellNr,
                                const char (*SpellStr)[512]) {
  ...
  switch (SpellNr) {
  case 34:
    CreateThing(Actor, SpellStr[2], SpellStr[3]);
    break;
  case 35:
    CreateThing(Actor, SpellStr[2], NULL);
    break;
  case 37:
    Teleport(Actor, SpellStr[2]);
    break;
  case 40:
    CreateKnowledge(Actor, SpellStr[3], NULL);
    break;
  case 41:
    ChangeData(Actor, SpellStr[2]);
    break;
  case 46:
    CreateKnowledge(Actor, SpellStr[3], SpellStr[4]);
    break;
  case 47:
    TeleportToCreature(Actor, SpellStr[3]);
    break;
  case 52:
    TeleportPlayerToMe(Actor, SpellStr[3]);
    break;
  case 53:
    SummonCreature(Actor, 0, SpellStr[3], true);
    break;
  case 58:
    GetPosition(Actor);
    break;
  case 63:
    CreateMoney(Actor, SpellStr[3]);
    break;
  case 64:
    ChangeProfession(Actor, SpellStr[3]);
    break;
  case 71:
    EditGuests(Actor);
    break;
  case 72:
    ...
    break;
  case 73:
    KickGuest(Actor, SpellStr[3]);
    break;
  case 74:
    EditNameDoor(Actor);
    break;
  case 96:
    GetQuestValue(Actor, SpellStr[3]);
    break;
  case 97:
    SetQuestValue(Actor, SpellStr[3], SpellStr[4]);
    break;
  case 98:
    CleanupField(Actor);
    break;
  case 99:
    MagicClimbing(Actor, SpellStr[3]);
    break;
  case 100:
    ClearQuestValues(Actor);
    break;
  case 101:
    KillAllMonsters(Actor, EFFECT_DEATH, 2);
    break;
  case 102:
    StartMonsterraid(Actor, SpellStr[4]);
    break;
  }
}

static void AccountRightSpell(uint32 CreatureID, int SpellNr,
                              const char (*SpellStr)[512]) {
  TCreature *Actor = GetCreature(CreatureID);
  if (Actor == NULL) {
  ...
  switch (SpellNr) {
  case 57:
    BanishAccount(Actor, SpellStr[3], atoi(SpellStr[4]), SpellStr[5]);
    break;
  case 60:
    HomeTeleport(Actor, SpellStr[2]);
    break;
  case 61:
    DeleteAccount(Actor, SpellStr[4], SpellStr[5]);
    break;
  case 62:
    SetNameRule(Actor, SpellStr[2]);
    break;
  case 65:
    Notation(Actor, SpellStr[2], SpellStr[3]);
    break;
  case 66:
    NameLock(Actor, SpellStr[3]);
    break;
  case 67:
    KickPlayer(Actor, SpellStr[2]);
    break;
  case 68:
    DeleteCharacter(Actor, SpellStr[4], SpellStr[5]);
    break;
  case 69:
    IPBanishment(Actor, SpellStr[3], SpellStr[4]);
    break;
  case 70:
    BanishCharacter(Actor, SpellStr[3], atoi(SpellStr[4]), SpellStr[5]);
    break;
  }
}

static void CastSpell(uint32 CreatureID, int SpellNr,
                      const char (*SpellStr)[512]) {
  TCreature *Actor = GetCreature(CreatureID);
  if (Actor == NULL) {
  ...
  case 13: {
    ...
    AngleCombat(Actor, ManaPoints, SoulPoints, Damage, EFFECT_ENERGY, 5, 30,
                DAMAGE_ENERGY);
    break;
  }
  ...
  case 19: {
    ...
    AngleCombat(Actor, ManaPoints, SoulPoints, Damage, EFFECT_FIRE_BURST, 4, 45,
                DAMAGE_FIRE);
    break;
  }
  ...
  case 22: {
    ...
    AngleCombat(Actor, ManaPoints, SoulPoints, Damage, EFFECT_FIRE_BLAST, 5, 0,
                DAMAGE_ENERGY);
    break;
  }
  ...
  case 23: {
    ...
    AngleCombat(Actor, ManaPoints, SoulPoints, Damage, EFFECT_FIRE_BLAST, 8, 0,
                DAMAGE_ENERGY);
    break;
  }
  ...
  case 80: {
    ...
    MassCombat(Actor, Actor->CrObject, Level * 4, 0, Damage, EFFECT_BONE_HIT, 2,
               DAMAGE_PHYSICAL, ANIMATION_NONE);
    break;
  }
  ...
  case 87: {
    ...
    AngleCombat(Actor, ManaPoints, SoulPoints, Damage, EFFECT_DEATH, 1, 0,
                DAMAGE_PHYSICAL);
    break;
  }
  ...
  case 88: {
    int Damage = ComputeDamage(Actor, SpellNr, 45, 10);
    AngleCombat(Actor, ManaPoints, SoulPoints, Damage, EFFECT_ENERGY, 1, 0,
                DAMAGE_ENERGY);
    break;
  }
  ...
  case 89: {
    int Damage = ComputeDamage(Actor, SpellNr, 45, 10);
    AngleCombat(Actor, ManaPoints, SoulPoints, Damage, EFFECT_FIRE_BURST, 1, 0,
                DAMAGE_FIRE);
    break;
  }
  ...
  case 104: {
    int Damage = ComputeDamage(Actor, SpellNr, 150, 50);
    AngleCombat(Actor, ManaPoints, SoulPoints, Damage, EFFECT_FIRE, 5, 30,
                DAMAGE_FIRE);
    break;
  }
...
static void GetSpellString(int SpellNr, char *Text) {
  ...
  if (SpellNr < 1 || SpellNr >= NARRAY(SpellList)) {
    error("GetSpellString: Invalid magic spell number %d.\n", SpellNr);
    return;
  }
...
void GetMagicItemDescription(Object Obj, char *SpellString, int *MagicLevel) {
  ...
  *MagicLevel = 0;
  ...
  if (SpellNr == 0) {
    error("GetMagicItemDescription: Object %d has no magic spell.\n",
          ObjType.TypeID);
    return;
  }
...
void GetSpellbook(uint32 CharacterID, char *Buffer) {
  ...
  for (int Level = 1; Level <= MaxLevel; Level += 1) {
    ...
    for (int SpellNr = 0; SpellNr < NARRAY(SpellList); SpellNr += 1) {
      ...
      if (Help[0] == 0) {
        error("GetSpellbook: Magic spell %d has no spell formula.\n",
              SpellNr);
        continue;
      }
...
int CheckForSpell(uint32 CreatureID, const char *Text) {
  ...
  while (!IS.eof()) {
    ...
    for (int SyllableNr = 0; SyllableNr < NARRAY(SpellSyllable);
         SyllableNr += 1) {
...
void UseMagicItem(uint32 CreatureID, Object Obj, Object Dest) {
  ...
  if (!Dest.exists()) {
    error("UseMagicItem: Passed target does not exist (Objekt %d).\n",
  ...
  int SpellNr = FindSpell(Obj);
  if (SpellNr == 0) {
    error("UseMagicItem: Object %d has no spell.\n",
          Obj.getObjectType().TypeID);
    throw ERROR;
  }
  ...
  try {
    ...
    case 7: {
      ...
      Combat(Actor, Dest, 0, 0, Damage, EFFECT_FIRE_BLAST, ANIMATION_FIRE,
             DAMAGE_ENERGY);
      break;
    }
    ...
    case 8: {
      int Damage =
          ComputeDamage(Actor, SpellNr, 45, 15); // 1.5x: 30->45, 10->15
      Combat(Actor, Dest, 0, 0, Damage, EFFECT_FIRE_BLAST, ANIMATION_FIRE,
             DAMAGE_ENERGY);
      break;
    }
    ...
    case 15: {
      ...
      MassCombat(Actor, Dest, 0, 0, Damage, EFFECT_FIRE_BURST, 3, DAMAGE_FIRE,
                 ANIMATION_FIRE);
      break;
    }

    case 16: {
      int Damage = ComputeDamage(Actor, SpellNr, 75,
                                 23); // 1.5x: 50->75, 15->23 (rounded)
      MassCombat(Actor, Dest, 0, 0, Damage, EFFECT_FIRE_BURST, 4, DAMAGE_FIRE,
                 ANIMATION_FIRE);
      break;
    }
    ...
    case 18: {
      ...
      MassCombat(Actor, Dest, 0, 0, Damage, EFFECT_EXPLOSION, 1,
                 DAMAGE_PHYSICAL, ANIMATION_FIRE);
      break;
    }

    case 21: {
      int Damage = ComputeDamage(Actor, SpellNr, 150, 20);
      Combat(Actor, Dest, 0, 0, Damage, EFFECT_DEATH, ANIMATION_DEATH,
             DAMAGE_PHYSICAL);
      break;
    }
    ...
    case 50: {
      ...
      Combat(Actor, Dest, 0, 0, Damage, EFFECT_FIRE, ANIMATION_FIRE,
             DAMAGE_FIRE_PERIODIC);
      break;
    }
    ...
    case 77: {
      ...
      Combat(Actor, Dest, 0, 0, Damage, EFFECT_POISON_HIT, ANIMATION_ENERGY,
    ...
    case 36: {
      // Mortal Strike
      // Base damage 1.5x that of explosion runes with same variation
      int Damage = ComputeDamage(Actor, SpellNr, 90, 40);
    ...
    case 43: {
      ...
      int Damage = ComputeDamage(Actor, SpellNr, 75,
                                 23); // Same enhanced damage as Great Fireball
      MassCombat(Actor, Dest, 0, 0, Damage, EFFECT_ENERGY, 4, DAMAGE_ENERGY,
                 ANIMATION_ENERGY);
      break;
    }
    ...
    case 105: {
      ...
      int Damage = ComputeDamage(Actor, SpellNr, 75,
                                 23); // Same enhanced damage as Great Fireball
      MassCombat(Actor, Dest, 0, 0, Damage, EFFECT_POISON_HIT, 4, DAMAGE_POISON,
                 ANIMATION_POISON);
      break;
    }
    ...
    case 106: {
      ...
      MassCombat(Actor, Dest, 0, 0, Damage, EFFECT_DARK_CLOUD, 4,
                 DAMAGE_PHYSICAL, ANIMATION_DEATH);
      break;
    }
    ...
    default: {
      error("UseMagicItem: Spell %d is not yet implemented.\n", SpellNr);
      throw ERROR;
    }
    }
  } catch (RESULT r) {
...
void DrinkPotion(uint32 CreatureID, Object Obj) {
  ...
  ObjectType ObjType = Obj.getObjectType();
  if (!ObjType.getFlag(LIQUIDCONTAINER)) {
    error("DrinkPotion: Passed Object is no liquid container.\n");
    throw ERROR;
  }
  ...
  if (LiquidType == LIQUID_MANA) {
    ...
    error("DrinkPotion: Objectdoes not contain a potion.\n");
    throw ERROR;
  }
    ...
    for (int SyllableNr = 0; SyllableNr < NARRAY(SpellSyllable);
      ...
      if (strcmp(SpellSyllable[SyllableNr], Syllable) == 0) {
        ...
        if (SyllableCount > NARRAY(Spell->Syllable)) {
          error("CreateSpell: Syllable count exceeded at Spell %d.\n",
                SpellNr);
