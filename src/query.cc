...
void TQueryManagerConnection::disconnect(void){
	if(this->Socket != -1){
		if(close(this->Socket) == -1){
			error("TQueryManagerConnection::disconnect: Error %d while closing socket.\n", errno);
		}
		this->Socket = -1;
	}
}
...
int TQueryManagerConnection::executeQuery(int Timeout, bool AutoReconnect){
	...
	if(!this->QueryOk){
		error("TQueryManagerConnection::executeQuery: Error while assembling request.\n");
		return QUERY_STATUS_FAILED;
	}
	...
	for(int Attempt = 1; true; Attempt += 1){
		...
		if(this->write(this->Buffer, PacketSize) != PacketSize){
			...
			if(Attempt >= MaxAttempts){
				error("TQueryManagerConnection::executeQuery: Error while submitting request.\n");
				return QUERY_STATUS_FAILED;
			}
			continue;
		}
		...
		if(this->read(this->Buffer, ResponseSize, Timeout) != ResponseSize){
			this->disconnect();
			error("TQueryManagerConnection::executeQuery: Error while reading data.\n");
			return QUERY_STATUS_FAILED;
		}
